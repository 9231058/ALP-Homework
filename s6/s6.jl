...
@constraint(m, s[1, 1] + x[1, 2, 2] + x[2, 2, 2] == s[1, 2] + 8000)
@constraint(m, s[2, 1] + x[1, 2, 2] + x[2, 2, 2] == s[2, 2] + 4000)
@constraint(m, s[1, 2] >= 1000)
@constraint(m, s[2, 2] >= 1000)
@objective(m, Min, 5 * (x[1, 1, 1] * 0.15 + x[1, 1, 2] * 0.12 + x[2, 1, 1] * 0.16 + x[2, 1, 2] * 0.14 + x[1, 2, 1] * 0.15 + x[1, 2, 1] * 0.12 + x[2, 2, 1] * 0.16 + x[2, 2, 2] * 0.14) + 0.2 * (s[1, 1] + s[2, 1]))
println(m)
